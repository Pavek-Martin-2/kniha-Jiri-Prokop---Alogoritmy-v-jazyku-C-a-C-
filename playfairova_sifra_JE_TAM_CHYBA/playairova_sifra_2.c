...
// playfairova sifra, plus moje upravy

#define DELKA 24
char abc[25]="abcdefghijklmnoprstuvwxyz"; // "q" je vynechano

// edit zona
char text [DELKA] = "tohlechcivsemlidemutajit"; // vse pouze malejma pismenama !
...
printf("bez cislic a pouze mala pismena\n"); // moje
printf("12345678901234567890\n"); // moje
j=strlen(gets(heslo));
printf("delka zadaneho hesla = %d",j); // pridano mnou
printf("\n\n");

printf("sifrovat se bude text:\n");
printf("%s",text);
printf("\n\n");
...
{
tab[k][m]=heslo[i];
if(heslo[i]<113) // kvuli vynechanemu pismenu q
abc[heslo[i]-97]=' ';
else abc[heslo[i]-98]=' ';
m++; if(m>4) {k++; m=0;}
}
...
sifruj();
// vypis vysledku - je v poli sifra
